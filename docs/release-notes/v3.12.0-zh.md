...
访问 [Releases](https://github.com/farion1231/cc-switch/releases/latest) 下载正式的 Web 运行时资产。
...
- 默认地址：`http://127.0.0.1:17666`
- 自定义端口：`CC_SWITCH_PORT=8080`
- 自定义监听地址：`CC_SWITCH_HOST=0.0.0.0`
- Linux 基线：Ubuntu 20.04+
- macOS 资产未签名，首次启动可能需要先移除隔离属性

本 fork 的正式发布资产不包含桌面 Tauri 安装包。
