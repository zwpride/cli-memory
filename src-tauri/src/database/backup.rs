...
const CLI_MEMORY_SQL_EXPORT_HEADER: &str = "-- CLI Memory SQLite 导出";
...
impl Database {
    ...
    fn import_sql_string_inner(
        ...
        let sql_content = sql_raw.trim_start_matches('\u{feff}');
        Self::validate_cli_memory_sql_export(sql_content)?;
    ...
    fn validate_cli_memory_sql_export(sql: &str) -> Result<(), AppError> {
        let trimmed = sql.trim_start();
        if trimmed.starts_with(CLI_MEMORY_SQL_EXPORT_HEADER) {
            return Ok(());
        }

        Err(AppError::localized(
            "backup.sql.invalid_format",
            "仅支持导入由 CLI Memory 导出的 SQL 备份文件。",
            "Only SQL backups exported by CLI Memory are supported.",
        ))
    }
...
mod tests {
    ...
    fn periodic_maintenance_runs_even_when_auto_backup_disabled() -> Result<(), AppError> {
        ...
        let test_home =
            std::env::temp_dir().join("cli-memory-periodic-maintenance-backup-disabled-test");
